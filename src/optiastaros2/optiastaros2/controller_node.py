...
class ControllerNode(Node):
    ...
    def __init__(self):
        ...
        self.targetPosArr, stop_pos, stop_orient = (generateRobotPath.pointsFromDoc(svg_str,density=0.05, scale=1)) # set density and scale of path
    ...
    def pose_callback(self, msg: RigidBody):
        ...
        for j in range(0,len(self.targetPosArr)):
            if self.targetPosArr[j]:
                ...
                # calculate angle
                self.angle[j] = pure_pursuit(self.currentPos[j],self.targetPosArr[j], currentHeading, lookahead_distance = 10) # lookahead is in indeces
