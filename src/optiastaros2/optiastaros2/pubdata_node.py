...
class PublishDataNode(Node):
    ...
    def __init__(self):
        ...
        streamingClient.run()
        self.timer_ = self.create_timer(0.01, self.send_data) #0.001
    ...
    def send_data(self):
        for i, publisher in enumerate(self.set_publishers):
            ...
            if str(i) in self.id_dict.keys():
                ...
                # debug for robot0:
                if str(i) == "0":
                    print("Robot0 (x,z,rot,deg,newY,newdeg): " + str(msg.pose.x) + " " + str(msg.pose.z) + " " + str(msg.rot.y) + " " + str(math.degrees(msg.rot.y)) + " " + str(y_angle) + " " + str(math.degrees(msg.rot.y)))
